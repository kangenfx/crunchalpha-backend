...
func (h *Handler) ReceiveTrade(c *gin.Context) {
        ...
        // ✅ REMOVED account-level check - GetAccountIDByNumber already verifies user ownership
        ...
        // TODO: Trigger AlphaRank recalculation if closed trade
        if data.Status == "closed" {
                // Async job to recalculate AlphaRank
...
func (h *Handler) ReceiveAccount(c *gin.Context) {
	...
	// Save equity snapshot for Real DD calculation
...
func (h *Handler) SyncAccount(c *gin.Context) {
	...
	// Save equity snapshot for floating DD calculation
	...
	// Save deposit/withdrawal data to account_transactions table
	...
	// Save all closed trades
        ...
        // Trigger AlphaRank calculation with all historical data
        go h.repo.TriggerAlphaRankCalculation(accountID)
