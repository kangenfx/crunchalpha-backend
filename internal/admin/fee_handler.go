...
func (h *FeeHandler) UpsertFeeOverride(c *gin.Context) {
	...
	err = h.DB.QueryRow(`
		INSERT INTO user_fee_overrides
		    (user_id, performance_fee, signal_fee_monthly, platform_fee_monthly, note)
		VALUES ($1, $2, $3, $4, $5)
		ON CONFLICT (user_id) DO UPDATE SET
		    ...
		    updated_at          = NOW()
		RETURNING id
	`, userID, req.PerformanceFee, req.SignalFeeMonthly, req.PlatformFeeMonthly, req.Note).Scan(&id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *FeeHandler) GetDefaultFees(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		...
		"signal_fee_monthly":   29.0,
		"platform_fee_monthly": 1.0,
	})
}
